...
def compress_mesh(vertices, uvs, normals, face_groups):
    ...
    # compress components
    sorted_indices = [
        sorted(zip(list(UIs.values()), list(UIs.keys()))) for UIs in used_indices
    ]
...
"""
    ...
    def write_context(self, context):
        ...
        aspect_ratio = float(width * context.aspectRatioX()) / (
            height * context.aspectRatioY()
        )
        samples_per_pixel = (1, context.OSALevel)[context.oversampling]
        out_file = (
            "//"
            + os.path.basename(os.path.splitext(self.script_path)[0])
            + image_types.get(context.imageType, ".jpg")
        )
        options = ""
    ...
    def write_body(self):
        ...
        for i, obj in enumerate(scene.objects):
            ...
            if obj_type in object_writers:
                ...
                print(
                    ("WARNING: Did not recognize object type '%(obj_type)s'" % vars())
                )
...
"""
    ...
    def draw_progress(self, progress):
        Blender.Window.DrawProgressBar(
            float(progress), "Exporting scene to LiAR [%3d%%]" % int(100 * progress)
        )
