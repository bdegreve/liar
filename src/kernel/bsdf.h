...
struct BsdfOut
{
	Spectrum value;
	TScalar pdf;
	BsdfOut(const Spectrum& value = Spectrum(), TScalar pdf = 0): value(value), pdf(pdf) {}
...
struct SampleBsdfOut
{
	...
	TScalar pdf;
	TBsdfCaps usedCaps;
	SampleBsdfOut(const TVector3D& omegaOut = TVector3D(), const Spectrum& value = Spectrum(), TScalar pdf = 0, TBsdfCaps usedCaps = 0):
