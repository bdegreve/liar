...
PY_CLASS_CONSTRUCTOR_2(Uv, TTextureRef, TTextureRef);

// --- public --------------------------------------------------------------------------------------

Uv::Uv(const TTextureRef& a, const TTextureRef& b):
	BinaryOperator(a, b)
{
}
...
const Spectral Uv::doLookUp(const Sample& sample, const IntersectionContext& context, SpectralType type) const
{
	...
	return Spectral(u * textureA()->lookUp(sample, context, SpectralType::Illuminant) + v * textureB()->lookUp(sample, context, SpectralType::Illuminant), type);
}
