...
def triangulate_3d(*verts):
    ...
    try:
        ...
        raise AssertionError(
            "Failed to triangulate %(verts)r, normal=%(normal)r: %(error)s" % vars()
        )
...
def polygon_normal_3d(*verts):
    ...
    return reduce(
        add, [cross_product(subtract(b, a), subtract(c, a)) for b, c in pairs]
    )
...
def triangulate_2d(*verts):
    ...
    if area == 0:
        return list(
            zip(
                vert_indices,
                vert_indices[1:] + vert_indices[:1],
                vert_indices[2:] + vert_indices[:2],
            )
        )
    triangles = []
    ...
    while len(vert_indices) > 3:
        ...
        for index in range(n):
            candidate = (
                vert_indices[index - 1],
                vert_indices[index],
                vert_indices[index + 1 - n],
            )
            ...
            if area <= 0:
                continue
            if any(
                in_triangle_2d(a, b, c, verts[k])
                for k in vert_indices
                if not k in candidate
            ):
                continue
...
def signed_area_2d(*verts):
    ...
    a = verts[0]
    pairs = zip(verts[1:-1], verts[2:])
    return 0.5 * sum(perp_dot(subtract(b, a), subtract(c, a)) for b, c in pairs)
