...
def decode(
    lines, filename="", material=liar.shaders.AshikhminShirley, initial_materials=None
):
    ...
    def _loadMaterials(filename, materials):
        ...
        for line in open(filename):
            ...
            def _enforceFields(iSuccess):
                assert iSuccess, "material libary %s has syntax error: %s" % (
                    filename,
                    line,
                )
            ...
            if len(fields) > 0:
                ...
                if command == "newmtl":
                    ...
                    if not name in materials:
                        ...
                        currentMaterial = (
                            None  # respect the one that's already there ...
                        )
                elif command == "Kd":
                    _enforceFields(len(fields) == 3)
                    if currentMaterial:
                        try:
                            currentMaterial.diffuse = liar.textures.Constant(
                                liar.rgb(_floatTuple(fields))
                            )
                        except AttributeError:
                            pass
                elif command == "Ks":
                    ...
                    if currentMaterial:
                        try:
                            currentMaterial.specular = liar.textures.Constant(
                                liar.rgb(_floatTuple(fields))
                            )
                        except AttributeError:
                            pass
                    ...
                    if currentMaterial:
                        _setSpecularPower(
                            currentMaterial, liar.textures.Constant(float(fields[0]))
                        )
                elif command == "map_Kd":
                    _enforceFields(len(fields) == 1)
                    if currentMaterial:
                    ...
                    if currentMaterial:
                        _setSpecularPower(
                            currentMaterial=liar.textures.Image(fields[0])
                        )
    ...
    def _makeGroup(vertices, normals, uvs, faces, material):
        ...
        sorted_components = [
            sorted(zip(UCs.values(), UCs.keys())) for UCs in used_components
        ]
        compressed_components = [
            [Cs[j] for i, j in SCs] for Cs, SCs in zip(components, sorted_components)
        ]
        ...
        for UCs in used_components:
            UCs[None] = None
        faces = [
            [[UCs[i] for i, UCs in zip(vertex, used_components)] for vertex in face]
            for face in faces
        ]

        group = liar.scenery.TriangleMesh(
            compressed_components[0],
            compressed_components[1],
            compressed_components[2],
            faces,
        )
        ...
        if len(fields) > 0:
            ...
            if command == "mtllib":
                ...
                if name not in materials.keys():
                    print(
                        "obj file %s uses unknown material '%s', using default instead: %s"
                        % (filename, name, line)
                    )
                    name = ""
                ...
                if len(faces) > 0:
                    numFaces += len(faces)
                    groups.append(
                        _makeGroup(vertices, normals, uvs, faces, currentMaterial)
                    )
                    faces = []
                        ...
                        if vertex[k] != None:
                            if vertex[k] > 0:
                                ...
                                raise AssertionError(
                                    "obj file %s has 0 index on line: %s"
                                    % (filename, line)
                                )
                ...
                try:
                    ...
                    raise AssertionError(
                        "failed to triangulate %(face)r: %(error)s" % vars()
                    )
    ...
    print(
        "%s vertices, %s normals, %s uvs, %s faces"
        % (len(vertices), len(normals), len(uvs), numFaces)
    )
