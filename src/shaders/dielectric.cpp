...
const TTexturePtr& Dielectric::colour() const
{
	return colour_;
}
...
void Dielectric::setColour(const TTexturePtr& colour)
{
	colour_ = colour;
}
		...
		const SampleBsdfIn* first, const SampleBsdfIn* last, SampleBsdfOut* result) const
{
	...
	const Spectrum colour = colour_->lookUp(sample, context);
...
{
	return python::makeTuple(outerRefractionIndex_, innerRefractionIndex_, colour_);
}
...
{
	python::decodeTuple(state, outerRefractionIndex_, innerRefractionIndex_, colour_);
}
