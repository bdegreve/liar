...
const Spectral LightSky::doEmission(const Sample& sample, const TRay3D& ray, BoundedRay& shadowRay, TScalar& pdf) const
{
	...
	return Spectral(radianceMap_[ii * resolution_.y + jj]);
}
		...
		BoundedRay& emissionRay, TScalar& pdf) const
{
	...
	return Spectral(radianceMap_[static_cast<size_t>(num::floor(i)) * resolution_.y + static_cast<size_t>(num::floor(j))]);
}



const Spectral LightSky::doTotalPower() const
{
	return Spectral(power_);
}
