...
def main(numBins, minWavelength, maxWavelength, rgbSpaceName, niter=100000):
    ...
    white, yellow, magenta, cyan, red, green, blue = generate_base_spectra(rgbObserver, niter)

    write_code_fragments(wavelengthBins, rgbSpace, niter, xyzObserver, white, yellow, magenta, cyan, red, green, blue)
...
def xyz_to_rgb(A, rgbSpace):
    # TODO: use rgbSpace to convert directly

    print rgbSpace.__getstate__()
    rx, ry = rgbSpace.red
    gx, gy = rgbSpace.green
    bx, by = rgbSpace.blue
    wx, wy = rgbSpace.white
    wY = 1
    ...
    toXYZ = scipy.array([[rx * Cr, gx * Cg, bx * Cb],
                         [ry * Cr, gy * Cg, by * Cb],
                         [(1.0 - (rx + ry)) * Cr, (1.0 - (gx + gy)) * Cg, (1.0 - (bx + by)) * Cb]])
    XYZtoRGB = scipy.linalg.inv(toXYZ)

    return scipy.dot(XYZtoRGB, A)


def generate_base_spectra(A, niter):
    print A
    null_A = null(A)
    pinv_A = scipy.linalg.pinv(A)

    white   = fit(1, 1, 1, null_A, pinv_A, niter)
    yellow  = fit(1, 1, 0, null_A, pinv_A, niter)
    magenta = fit(1, 0, 1, null_A, pinv_A, niter)
    cyan    = fit(0, 1, 1, null_A, pinv_A, niter)
    red     = fit(1, 0, 0, null_A, pinv_A, niter)
    green   = fit(0, 1, 0, null_A, pinv_A, niter)
    blue    = fit(0, 0, 1, null_A, pinv_A, niter)
...
def write_code_fragments(wavelengthBins, rgbSpace, niter, A, white, yellow, magenta, cyan, red, green, blue):
    ...
    meta_info = [
        "// Generated by %s with %d bins from %s to %s, using %s iterations\n" % (os.path.basename(__file__), n, wavelengthBins[0], wavelengthBins[-1], niter)
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--niter", type=int, default=100000, metavar='<n>', help="number of iterations in optimization algorithm, more is better [default=%(default)s]")
    ...
    main(args.num_bins, args.w_min, args.w_max, args.rgb_space, args.niter)
